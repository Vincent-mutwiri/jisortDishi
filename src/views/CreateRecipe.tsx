...
const BasicInfoStep = ({ recipe, updateRecipe }: any) => {
  ...
  const updateTitle = (value: string) => { setLocalTitle(value); updateRecipe('title', value); };
  const updateDescription = (value: string) => { setLocalDescription(value); updateRecipe('description', value); };
        ...
        <div>
          ...
          <div className="relative">
            ...
            >
              <span className="text-[#1a1a1a]">
                {recipe.category && recipe.category.split(',').filter((c: string) => c.trim()).length > 0 
                  ? `${recipe.category.split(',').filter((c: string) => c.trim()).length} meal type(s) selected`
                  : 'Choose meal types...'
                }
              </span>
              <ChevronDown size={20} className={`text-[#4a4a3a] transition-transform ${isDropdownOpen ? 'rotate-180' : ''}`} />
            </button>
            
            {isDropdownOpen && (
              <div
                ref={dropdownRef}
                className="absolute z-10 w-full mt-1 bg-white border border-[#eaeaE0] rounded-lg shadow-lg max-h-64 overflow-y-auto"
              >
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (e.target.checked) {
                          ...
                          updateRecipe('category', current.filter((c: string) => c !== category.value).join(','));
                        }
                      }}
          ...
          {recipe.category && recipe.category.split(',').filter((c: string) => c.trim()).length > 0 && (
            <div className="mt-3 flex flex-wrap gap-2">
              {recipe.category.split(',').filter((c: string) => c.trim()).map((selectedCategory: string) => {
                ...
                return category ? (
                  ...
                  >
                    ...
                    <button
                      ...
                      onClick={() => {
                        const current = recipe.category ? recipe.category.split(',') : [];
                        updateRecipe('category', current.filter((c: string) => c !== selectedCategory).join(','));
                      }}
        ...
        <div key={index} className="flex flex-col gap-2 p-4 border border-[#eaeaE0] rounded-xl bg-[#fafaf8]">
          ...
          <div className="flex gap-2 items-center">
            <input
              ...
              placeholder="Ingredient name (e.g. Flour, Tomatoes)"
              className="flex-1 px-3 py-2 border border-[#eaeaE0] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#5A5A40] bg-white"
            />
            <button
          ...
          <div className="flex flex-wrap gap-2 items-center">
            <div className="flex gap-1.5 items-center">
              <input
                ...
                min="0"
                step="0.1"
                className="w-20 px-3 py-2 border border-[#eaeaE0] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#5A5A40] bg-white text-sm"
              />
              <label className="text-xs text-[#9e9e9e]">Qty</label>
            </div>
            <div className="flex gap-1.5 items-center">
              <select
                value={ingredient.unit}
                onChange={(e) => updateIngredient(index, 'unit', e.target.value)}
                className="w-28 px-3 py-2 border border-[#eaeaE0] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#5A5A40] bg-white text-sm"
              >
              ...
              <label className="text-xs text-[#9e9e9e]">Unit</label>
            </div>
            <div className="flex gap-1.5 items-center">
              <input
                type="number"
                value={ingredient.costPerUnit ?? ''}
                onChange={(e) => updateIngredient(index, 'costPerUnit', e.target.value ? parseFloat(e.target.value) : undefined)}
                placeholder="Cost/unit"
                min="0"
                step="0.01"
                className="w-28 px-3 py-2 border border-[#eaeaE0] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#5A5A40] bg-white text-sm"
              />
              <label className="text-xs text-[#9e9e9e]">Price/unit</label>
            </div>
            {ingredient.costPerUnit && ingredient.quantity ? (
              <span className="text-xs font-bold text-[#5A5A40] bg-[#f0f0eb] px-2 py-1 rounded-lg ml-auto">
                = {(ingredient.costPerUnit * ingredient.quantity).toLocaleString()}
              </span>
            ) : null}
          </div>
          ...
          <input
            ...
            className="px-3 py-2 border border-[#eaeaE0] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#5A5A40] bg-white text-sm text-[#9e9e9e]"
          />
        </div>
      ))}
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-wrap gap-2 mb-3">
          {recipe.tags.map((tag: string, index: number) => (
            ...
            >
              {tag}
              <button onClick={() => removeTag(tag)} className="hover:text-[#eaeaE0]">
                <X size={14} />
              </button>
            </span>
          ))}
    ...
    <div className="bg-white rounded-2xl p-4 sm:p-6 border border-[#eaeaE0]">
      <div className="text-center mb-6">
        ...
        <div className="flex flex-wrap justify-center gap-2 sm:gap-4 text-xs sm:text-sm text-[#4a4a3a]">
          <span className="flex items-center gap-1"><Clock size={16} />{recipe.prep_time_minutes + recipe.cook_time_minutes} min total</span>
          <span className="flex items-center gap-1"><Users size={16} />{recipe.servings} people</span>
          <span className="px-3 py-1 bg-[#5A5A40] text-white rounded-full text-xs">{recipe.difficulty}</span>
        </div>
      </div>
      {recipe.ingredients.length > 0 && (
        <div className="mb-6">
      ...
      <div className="text-center pt-4 border-t border-[#eaeaE0]">
        <p className="text-sm text-[#9e9e9e]">{recipe.is_public ? '🌍 Everyone can see this recipe' : '🔒 Only you can see this recipe'}</p>
      </div>
    </div>
  </div>
);
...
export default function CreateRecipe() {
  ...
  const renderStepContent = () => {
    switch (currentStep) {
      ...
      case 5:
        return <ReviewStep recipe={recipe} />;
      default:
        return null;
    }
  };
